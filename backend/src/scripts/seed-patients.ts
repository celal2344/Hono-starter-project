...
async function seedPatients() {
    try {
        ...
        // --- Seed better-auth users so they are usable for login ---
        try {
            ...
            // Plaintext passwords for local/dev testing — change as needed
            const doctorPassword = 'Doctor123!';
            const nursePassword = 'Nurse123!';

            const seedUsers = [
                {
                    email: `dr.jones+${timestampStr}@example.com`,
                    name: 'Dr. Amelia Jones',
                    role: 'doctor',
                    password: doctorPassword,
                },
                {
                    email: `nurse.kim+${timestampStr}@example.com`,
            ...
            // Try to create users via better-auth API if available. Fall back to raw insert.
            ...
            for (const u of seedUsers) {
                try {
                    ...
                    if (api && typeof api.createUser === 'function') {
                        // common admin createUser signature
                    ...
                    } else if (api && typeof api.signUp === 'function') {
                        // some libs use signUp
                        created = await api.signUp({
                            email: u.email,
                            password: u.password,
                            name: u.name,
                        });
                        // if role must be set after signup, try updating user record
                        if (created && created.user && u.role) {
                            try {
                                if (typeof api.updateUser === 'function') {
                                    await api.updateUser(created.user.id || created.user._id, { role: u.role });
                                }
                            } catch { }
                        }
                    } else if (api && typeof api.create === 'function') {
                        created = await api.create({ email: u.email, password: u.password, name: u.name, role: u.role });
                    } else {
                        // Fallback: insert directly into users collection. This may not make the account usable
                        const raw = {
                            email: u.email,
                            name: u.name,
                            role: u.role,
                            authProvider: 'better-auth',
                            authProviderId: `seed-${u.role}-${timestampStr}`,
                            emailVerified: true,
                            // NOTE: storing plaintext password here for local/dev only — prefer better-auth API
                            password: u.password,
                            createdAt: Date.now(),
                            updatedAt: Date.now(),
                        } as any;
                        const r = await usersCollection.insertOne(raw);
                        created = { insertedId: r.insertedId };
                    }

                    // Push an identifier for logging
                    if (created) {
                        if (created.insertedId) createdUserIds.push(created.insertedId.toString());
                        else if (created.user && (created.user.id || created.user._id)) createdUserIds.push((created.user.id || created.user._id).toString());
                        else if (created.id) createdUserIds.push(created.id.toString());
                        else createdUserIds.push(u.email);
                    }

                    console.log(`✅ Seeded user ${u.email} (role=${u.role}) — password: ${u.password}`);
                } catch (innerErr) {
                    logger.error({ module: 'seed-patients', error: innerErr, user: u.email }, 'Failed to create user via auth API; skipping');
                    console.error(`❌ Failed to create user ${u.email}:`, innerErr);
                }
            }

            logger.info({ module: 'seed-patients', users: createdUserIds }, `Seeded ${createdUserIds.length} auth users`);
        } catch (uErr) {
            logger.error({ module: "seed-patients", error: uErr }, 'Error seeding better-auth users');
            console.error('❌ Error seeding better-auth users:', uErr);
            // don't rethrow here — patients were already inserted; continue to finally block
        }
    } catch (error) {
